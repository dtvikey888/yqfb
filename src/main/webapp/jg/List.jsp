...
String check="";
if(from!=null){
  check=Topay.WxJsApiCheck48(urlname,openid,lb,from);
}else{
  check=Topay.WxJsApiCheck46(urlname,openid,lb);
}
...
<script>
   ...
   */
        ...
        wx_share_link="<%=AllValus.propath %>jg/List.jsp?openid=<%=openid%>&lb=<%=lb%>";
...
String sql="";
if(lb==0){
sql="select * from jg where oid="+oid+" and yz<>0 order by id desc";
}else if(lb==1){
sql="select * from jg where lb=1 and yz<>0 order by id desc";
}else if(lb==2){
sql="select * from jg where lb=2 and yz<>0 order by id desc";
}else if(lb==3){
sql="select * from jg where lb=3 and yz<>0 order by id desc";
}else if(lb==4){
sql="select * from jg where lb=4 and yz<>0 order by id desc";
}
         ...
         <img src="<%=images2%>" width="100%;"/>
		  ...
		  <div class="div-b"><%=ZghTools.GetNickname(oid2)%></div> 
           ...
           // kz=1 开始抽奖(不关) kz=2 抽奖结束(关闭) kz=3 抽奖没开始（关闭）
            ...
            <%}else{%>
            <a href="/yqfb/del?action=5&id=<%=id%>&openid=<%=openid%>&lb=<%=lb%>&uprl=List.jsp">
