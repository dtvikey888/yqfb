...
%>			
        ...
        <tr height="35">
          <td><img src="CakeServlet?id=<%=id%>"></td>
          <td align="center">&nbsp;</td>
          <td align="center">&nbsp;</td>
          <td align="center">&nbsp;</td>
        </tr>
...
%>	
  ...
  <tr align="right"  class="biaoti">
    <td height="22" colspan="10" ><div align="right">第<%=intPage%>页/共<%=intPageCount%>页/<%=intRowCount %>条&nbsp;&nbsp;
          <%if(intPage>1){%>
      <a href="interface/index2.jsp?page=<%=intPage-1%>">上一页</a>
      <%}%>
  &nbsp;&nbsp;
  <%if(intPage<intPageCount){%>
  <a href="interface/index2.jsp?page=<%=intPage+1%>">下一页</a>
  <%}%>
 &nbsp;&nbsp;选择页码：
  <select name="page" onChange="MM_jumpMenu('this',this,0)" style="width:80px;">
    <option value="interface/index2.jsp?page=<%=intPage%>" selected><%=intPage%></option>
    <%int j;
	  for (j=1;j<=intPageCount;j++){%>
    <option value="interface/index2.jsp?page=<%=j%>"><%=j%></option>
    <%}%>
  </select>
