...
<%
//dl.jsp 用户数据填写
String urlname ="xm/dl.jsp";
String openid =request.getParameter("openid");
String check=Topay.WxJsApiCheck4(urlname,openid);
int fid = MyUtils4.GetFid();
boolean cz = MyUtils4.iscz(openid,fid);
if(cz){
  response.sendRedirect("cj.jsp?openid="+openid+"");
}
%>
...
<div class="body p-10 address-edit">
    ...
    <form id="form1" name="form1" method="post" action="cj.jsp">
...
<span gaevent="imt/account/address/addBtn" class="btn btn-block" id="btn-login" );">这次名额已经没了报名结束！</span>
		...
		-->
          <div class="form-btn">
  <button type="submit" gaevent="imt/account/address/addBtn" class="btn btn-block" id="btn-login" onClick="document.form1.action='cj.jsp';if(DataLength(form1.xm.value)<2){alert('姓名最少2个字');form1.xm.focus();return false};if(DataLength(form1.tel.value)<2){alert('请填写正确的联系方式');form1.tel.focus();return false};sharefriendRound2();sharefriendRound3();sharefriendRound4();sharefriendRound5();sharefriendRound6();document.form1.submit();">提交</button>
          </div>
          <!--
		end if%>
		--></td>
